/* file: icu.c */
 ...
 #pragma linesize(132)
 #pragma pagesize(63)
 #pragma title("Interrupt Control Unit (ICU) functions")
...
/****************************************************************************
 *
 *	Check Base Master/Slave interrupt Controllers
 *
 **************************************************************************/
...
/****************************************************************************
 *
 *      end of ICU function prototypes
 *
 ***************************************************************************/
 ...
 ***************************************************************************/
 ...
 #define icw1 ICW1_BASE | ICW1_NO_ICW4 | ICW1_SINGLE | ICW1_INTERVAL_8 | ICW1_EDGE_TRIGGERED
 #define icw2 ICU_BASE_IR
 #define icw4 ICW4_M8086 | ICW4_NORMAL_EOI | ICW4_NON_BUFFERED | ICW4_NOT_SPECIAL
 #define ocw1 IR0_MASK | IR1_MASK | IR2_MASK | IR3_MASK | IR4_MASK | IR5_MASK | IR6_MASK | IR7_MASK

 #elif defined(V20)

 #define icw1 ICW1_BASE | ICW1_YES_ICW4 | ICW1_SINGLE | ICW1_INTERVAL_8 | ICW1_EDGE_TRIGGERED
 #define icw2 ICU_BASE_IR
 #define icw4 ICW4_M8086 | ICW4_NORMAL_EOI | ICW4_BUFFERED_MASTER | ICW4_NOT_SPECIAL
 #define ocw1 IR0_MASK | IR1_MASK | IR2_MASK | IR3_MASK | IR4_MASK | IR5_MASK | IR6_MASK | IR7_MASK
...
/****************************************************************************
 *
 *	end of ICU data structure definitions
 *
 ***************************************************************************/
 ...
 ***************************************************************************/
	...
	install_vector(dummy_int_handler,IR0);  /* irq 0 */
						/* irq 1 used for KERNEL (V40/50) */
	install_vector(dummy_int_handler,IR2);  /* irq 2 */
	install_vector(dummy_int_handler,IR3);  /* irq 3 */
	install_vector(dummy_int_handler,IR4);  /* irq 4 */
	install_vector(dummy_int_handler,IR5);  /* irq 5 */
	install_vector(dummy_int_handler,IR6);  /* irq 6 */
 ...
 ***************************************************************************/
	...
	install_vector(incomplete_int_handler,IR7);

	irq_table.incomplete = 0;       /* initialize error count */
 ...
 ***************************************************************************/
	...
	outp(ICU_BASE,icw1);			/* edge triggered */
	outp(ICU_MASKPT,icw2);			/* base vector */
	...
	outp(ICU_BASE,icw1);            /* edge triggered */
	outp(ICU_MASKPT,icw2);          /* base vector */
	outp(ICU_MASKPT,icw4);          /* buffered master mode */
	outp(ICU_MASKPT,ocw1);          /* mask all interruputs (disable) */
	outp(ICU_BASE,OCW2_SET_PRIORITY | LOW_IRQ);
 ...
 *
 *
 *	GENERAL INTERRUPT VECTOR FUNCTIONS
 *
 *
 ***************************************************************************/
 ...
 *
 *
 *       DEFAULT INTERRUPT SERVICE FUNCTIONS
 *
 *
 ***************************************************************************/
