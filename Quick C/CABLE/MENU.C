...
MENU mnuAtrib =
{
    _TCYAN , _TCYAN , _TBLACK, _TBRIGHTWHITE, _TBRIGHTWHITE,
    _TBLACK, _TBLACK, _TCYAN , _TBLACK, _TCYAN ,
    TRUE,
    'Ú', '¿', 'Ù', 'À', '³', 'Ä'
};
...
int Menu( int row, int col, ITEM aItem[], int iCur )
{
    ...
    /* set limits on mouse */
    mouse_box( row, col, cItem, cchItem );
	...
	/* set mouse cursor to current line */
	mouseloc_text(col, row + iCur );
    }
...
void _outchar( char out )
{
 ...
 unsigned Get_User( int fWait )
 {
	unsigned uKey;	/* Unsigned key code */

	mouse_house();

	while( (uKey = GetKey( NO_WAIT)) == FALSE ) {
		mouse_house();
		if(mouse.row > mouse.row_last) {
			uKey = U_DN;
			break;
			}
		if(mouse.row < mouse.row_last) {
